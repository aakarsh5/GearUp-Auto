...
//creating endpoint for popular in electronics
app.get("/popularelectronics", async (req, res) => {
...
// Endpoint to fetch companies based on selected year
...
// Endpoint to fetch models based on selected year and company
app.get("/models/:year/:company", async (req, res) => {
  const { year, company } = req.params;
  try {
    const models = await Product.find({ year: parseInt(year), company });
    res.json(models);
  } catch (error) {
...
//creating endpoint to get cartdata while login
